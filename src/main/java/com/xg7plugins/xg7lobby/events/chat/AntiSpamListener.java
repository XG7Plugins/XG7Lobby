...
public class AntiSpamListener implements Listener {
    ...
    public void onChat(AsyncPlayerChatEvent event) {
        ...
        ConfigSection config = ConfigFile.mainConfigOf(XG7LobbyLoader.getInstance()).section("anti-spam");

        if (player.hasPermission("xg7lobby.chat.spam")) return;

        boolean antiSpamOnlyInLobby = config.get("anti-spam-only-in-lobby", false);
        if (antiSpamOnlyInLobby && !XG7Plugins.getAPI().isInAnEnabledWorld(XG7LobbyLoader.getInstance(), event.getPlayer())) return;
        ...
        if (XG7Plugins.getAPI().cooldowns().containsPlayer("lobby-chat-spam", player)) {
            ...
            Text.sendTextFromLang(player, XG7LobbyLoader.getInstance(), "chat.message-cooldown", Pair.of("time", cooldown + ""));
        ...
        if (messageCannotBeTheSame) {
            ...
            if (lastMessage != null && lastMessage.equalsIgnoreCase(message)) {
                Text.sendTextFromLang(player, XG7LobbyLoader.getInstance(), "chat.same-message");
                event.setCancelled(true);
                return;
            }
        ...
        AntiSpamTask antiSpamTask = (AntiSpamTask) XG7Plugins.getAPI().taskManager().getTimerTask(XG7LobbyLoader.getInstance(), "anti-spam-tolerance");
        ...
        if (antiSpamTask.getTolerance(player.getUniqueId()) >= sendWarningOnMessage) {
            Text.sendTextFromLang(player, XG7LobbyLoader.getInstance(), "chat.send-much-messages");
        }
    }
}
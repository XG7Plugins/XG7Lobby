...
public class LockChatCommandListener implements Listener {
    ...
    public void onChatWhenLock(AsyncPlayerChatEvent event) {
        XG7LobbyEnvironment xg7LobbyConfig = XG7LobbyLoader.getInstance().getEnvironmentConfig();

        if (event.getPlayer().hasPermission("xg7lobby.chat.ignore-lock")) return;
        if (xg7LobbyConfig.isChatLocked()) {

            if (ConfigFile.mainConfigOf(XG7LobbyLoader.getInstance()).root().get("lock-chat-only-in-lobby", false) && !XG7Plugins.getAPI().isInAnEnabledWorld(XG7LobbyLoader.getInstance(), event.getPlayer())) return;

            Text.sendTextFromLang(event.getPlayer(), XG7LobbyLoader.getInstance(), "chat.locked");

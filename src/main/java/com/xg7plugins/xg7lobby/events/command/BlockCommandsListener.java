...
public class BlockCommandsListener implements Listener {
    ...
    public boolean isEnabled() {
        return ConfigFile.mainConfigOf(XG7LobbyLoader.getInstance()).section("block-commands").get("enabled");
    }
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        ...
        ConfigSection config = ConfigFile.mainConfigOf(XG7LobbyLoader.getInstance()).section("block-commands");

        if (config.getList("blocked-commands", String.class).orElse(Collections.emptyList()).contains(event.getMessage().split(" ")[0])) {
            event.setCancelled(true);
            Text.fromLang(event.getPlayer(), XG7LobbyLoader.getInstance(), "chat.prohibited-command").thenAccept(text -> text.send(event.getPlayer()));
        }
    }
}
...
public class CollaboratorsForm extends SimpleForm {
    public CollaboratorsForm() {
        super("lobby-collaborators-help", "lang:[help.form.collaborators-title]", XG7LobbyLoader.getInstance());
    }
    ...
    public List<ButtonComponent> buttons(Player player) {
        ...
        buttons.add(ButtonComponent.of(Text.fromLang(player, XG7LobbyLoader.getInstance(), "help.form.click-to-back").join().getText()));
    ...
    public void onFinish(org.geysermc.cumulus.form.SimpleForm form, SimpleFormResponse result, Player player) {
        switch (result.clickedButtonId()) {
            ...
            case 4:
                ...
                return;
            case 5:
                XG7LobbyLoader.getInstance().getHelpMessenger().sendForm(player);
        }
    }
